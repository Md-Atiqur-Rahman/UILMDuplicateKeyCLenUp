...
public static class TestInconsistentDuplicateCleaner
{
        ...
        IMongoCollection<BsonDocument> deletedModulesCollection)
    {
        ...
        static (List<BsonDocument> Matched, List<BsonDocument> Unmatched) GroupModulesByReference(BsonArray modules)
        {
            ...
            else if (moduleList.Any(m => m.ModuleName == "generic-app"))
                referenceModuleName = "generic-app";
            else
                referenceModuleName = moduleList.First().ModuleName;
            ...
            foreach (var m in moduleList.OrderBy(m => m.ModuleName).ThenBy(m => m.Occurrence))
            {
                ...
                if (isMatch) matched.Add(m.Doc); else unmatched.Add(m.Doc);
            }
        ...
        foreach (var doc in results)
        {
            ...
            // Preserve existing behavior for consistent keys (A/B/C/D)
            if (isConsistent)
            {
                // --- FILTER A ---
                if (!hasRoot && !hasGeneric)
                {
                    ...
                    var consistentResources = firstModule.Contains("Resources") ? firstModule["Resources"].AsBsonArray : new BsonArray();
                    ...
                    var archivedRecord = new BsonDocument
                    {
                        ...
                        { "ConsistentDataExtractedFrom", firstModule.Contains("Module") ? firstModule["Module"] : BsonNull.Value }
                    };
                ...
                else if (!hasRoot && hasGeneric)
                {
                    ...
                    foreach (var module in modules)
                    {
                        var moduleName = module.AsBsonDocument.GetValue("Module").AsString;
                    ...
                    if (modulesToDelete.Count > 0)
                    {
                        var archivedRecord = new BsonDocument
                        {
                            ...
                            { "DuplicateGenericAppRemoved", modulesToDelete.Cast<BsonDocument>().Count(m => m["Module"].AsString == "generic-app") }
                        };
            ...
            // ----- NEW: isConsistent == false -----
            var (matchedGroup, unmatchedGroup) = GroupModulesByReference(modules);

            var finalModules = new BsonArray();
            var groupArchives = new List<BsonDocument>();

            // Helper to process each group and ensure one generic module remains (create if missing)
            void ProcessGroup(string label, List<BsonDocument> groupModules, bool suffixNonMatching)
            {
                if (groupModules == null || groupModules.Count == 0)
                    return;

                // Find existing generic-app inside the group
                var existingGeneric = groupModules.FirstOrDefault(m => m.Contains("Module") && m["Module"].AsString == "generic-app");

                BsonDocument genericToKeep;
                if (existingGeneric != null)
                {
                    genericToKeep = existingGeneric;
                }
                else
                {
                    // Build a new generic module using Resources from the first module in the group
                    var firstMod = groupModules[0];
                    var resources = firstMod.Contains("Resources") ? firstMod["Resources"].AsBsonArray : new BsonArray();

                    var moduleName = suffixNonMatching ? "generic-app-nonmatching" : "generic-app";

                    genericToKeep = new BsonDocument
                    {
                        { "Module", moduleName },
                        { "Id", ObjectId.GenerateNewId().ToString() },
                        { "Resources", resources }
                ...
                // Delete everything from the group except the genericToKeep
                var toDelete = new BsonArray();
                foreach (var m in groupModules)
                {
                    // Skip the kept generic (match by Module name and Id if available)
                    if (m.Contains("Module") && genericToKeep.Contains("Module") && m["Module"].AsString == genericToKeep["Module"].AsString)
                    {
                        // If it's the existingGeneric reference, skip deletion
                        if (ReferenceEquals(m, existingGeneric))
                            continue;

                        // If we created genericToKeep then the names differ only when suffixNonMatching true,
                        // so above name-check will not match — we will delete originals and keep created generic.
                    }

                    // For safety, if both have Id fields and they match, skip deletion
                    if (m.Contains("Id") && genericToKeep.Contains("Id") && m["Id"].AsString == genericToKeep["Id"].AsString)
                        continue;

                    toDelete.Add(m);
                }

                // Add kept generic to final modules (existing doc or newly created)
                finalModules.Add(genericToKeep);

                // Archive this group's deletions
                var archived = new BsonDocument
                {
                    ...
                    { "GenericModuleKept", genericToKeep }
                };

                groupArchives.Add(archived);
                deletedModuleCount += toDelete.Count;
            }

            // Process matching group (keep/create generic-app)
            ProcessGroup("Matching", matchedGroup, suffixNonMatching: false);

            // Process non-matching group (keep/create generic-app-nonmatching to avoid collision)
            ProcessGroup("NonMatching", unmatchedGroup, suffixNonMatching: true);

            // Save archives for both groups
            foreach (var ar in groupArchives)
                deletedRecords.Add(ar);

            // Update original document with final modules (kept generics)
            var updateDef = Builders<BsonDocument>.Update
                .Set("Modules", finalModules)
                .Set("UpdatedAt", DateTime.UtcNow)
                .Set("ModulesDeletionTriggered", true)
                .Set("FilterType", filterType)
                .Set("IsConsistent", false); // remains inconsistent but modules adjusted

            await collection.UpdateOneAsync(
                Builders<BsonDocument>.Filter.Eq("_id", doc["_id"]),
                updateDef);

            Console.WriteLine($"✅ [FILTER {filterType}] For inconsistent KeyName '{keyName}' kept {finalModules.Count} generic module(s), deleted {deletedModuleCount} modules.");
